...
#     def test_api_key_initialization_from_settings(self):
#         """Test API key initialization from settings when no config provided."""
#         # Mock the settings to return a test API key
#         with patch('lobster.tools.providers.geo_provider.get_settings') as mock_get_settings:
#             mock_settings = Mock()
#             mock_settings.NCBI_API_KEY = "test_api_key_123"
#             mock_get_settings.return_value = mock_settings
            
#             # Create provider without config - should get API key from settings
#             provider = GEOProvider(self.mock_data_manager, None)
            
#             # Verify the API key was set from settings
#             assert provider.config.api_key == "test_api_key_123"
    
#     def test_api_key_initialization_with_config_no_key(self):
#         """Test API key initialization when config provided but no API key."""
#         # Mock the settings to return a test API key
#         with patch('lobster.tools.providers.geo_provider.get_settings') as mock_get_settings:
#             mock_settings = Mock()
#             mock_settings.NCBI_API_KEY = "test_api_key_456"
#             mock_get_settings.return_value = mock_settings
            
#             # Create config without API key
#             config = GEOProviderConfig(api_key=None)
#             provider = GEOProvider(self.mock_data_manager, config)
            
#             # Verify the API key was set from settings
#             assert provider.config.api_key == "test_api_key_456"
    
#     def test_api_key_initialization_with_existing_key(self):
#         """Test API key initialization when config already has API key."""
#         # Mock the settings
#         with patch('lobster.tools.providers.geo_provider.get_settings') as mock_get_settings:
#             mock_settings = Mock()
#             mock_settings.NCBI_API_KEY = "settings_api_key"
#             mock_get_settings.return_value = mock_settings
            
#             # Create config with existing API key
#             config = GEOProviderConfig(api_key="existing_api_key")
#             provider = GEOProvider(self.mock_data_manager, config)
            
#             # Verify the existing API key was preserved
#             assert provider.config.api_key == "existing_api_key"
