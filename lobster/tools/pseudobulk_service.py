...
class PseudobulkService:
    ...
    def __init__(self, provenance_tracker: Optional[ProvenanceTracker] = None):
        ...
        """
        self.logger = logger
        self.provenance_tracker = provenance_tracker or ProvenanceTracker(namespace="lobster")
    ...
    def aggregate_to_pseudobulk(
        ...
        """
        # Log operation start
        self.logger.info(f"Starting pseudobulk aggregation: {adata.n_obs} cells → pseudobulk")
        
        try:
            # Validate inputs
            self._validate_aggregation_inputs(adata, sample_col, celltype_col, aggregation_method)
            ...
            grouping_df = self._create_grouping_dataframe(adata, sample_col, celltype_col)
            ...
            if min_genes > 0:
                pseudobulk_adata = self._filter_low_gene_samples(pseudobulk_adata, min_genes)
            ...
            if isinstance(e, (PseudobulkError, AggregationError, InsufficientCellsError)):
    ...
    def _validate_aggregation_inputs(
        ...
        if celltype_col not in adata.obs.columns:
            raise AggregationError(f"Cell type column '{celltype_col}' not found in obs")
        ...
        if sample_missing > 0:
            raise AggregationError(f"{sample_missing} cells have missing sample identifiers")
        
        if celltype_missing > 0:
            raise AggregationError(f"{celltype_missing} cells have missing cell type identifiers")
    ...
    def _get_expression_matrix(self, adata: anndata.AnnData, layer: Optional[str]) -> Union[np.ndarray, sparse.spmatrix]:
    ...
    def _filter_genes(
        ...
        self.logger.info(f"Filtered to {len(valid_genes)} genes from {len(gene_subset)} requested")
    ...
    def _create_grouping_dataframe(
        ...
        grouping_df['group_id'] = (
            grouping_df['sample_id'].astype(str) + '_' + 
    ...
    def _filter_groups_by_cell_count(
        ...
        valid_grouping_df = grouping_df[grouping_df['group_id'].isin(valid_groups.index)]
    ...
    def _perform_aggregation(
        ...
        group_metadata_df.index = [f"{row['sample_id']}_{row['cell_type']}" for _, row in group_metadata_df.iterrows()]
        
        self.logger.info(f"Aggregated {len(cell_indices)} cells into {n_groups} pseudobulk samples")
    ...
    def _create_pseudobulk_anndata(
        ...
        if 'sample_id' not in pseudobulk_adata.obs.columns:
            pseudobulk_adata.obs['sample_id'] = [idx.rsplit('_', 1)[0] for idx in pseudobulk_adata.obs.index]
        
        if 'cell_type' not in pseudobulk_adata.obs.columns:
            pseudobulk_adata.obs['cell_type'] = [idx.rsplit('_', 1)[1] for idx in pseudobulk_adata.obs.index]
    ...
    def _filter_low_gene_samples(self, adata: anndata.AnnData, min_genes: int) -> anndata.AnnData:
        ...
        if n_filtered > 0:
            self.logger.warning(f"Filtering {n_filtered} pseudobulk samples with <{min_genes} genes")
        ...
        try:
            ...
            for activity in self.provenance_tracker.activities:
                if activity['id'] == activity_id:
                    activity['inputs'] = [{"entity": input_entity_id, "role": "single_cell_data"}]
                    activity['outputs'] = [{"entity": output_entity_id, "role": "pseudobulk_matrix"}]
