...
class WorkflowTracker:
    ...
    def track_iteration(
        ...
        try:
            ...
            significant_genes = set(
                results_df[results_df["padj"] < parameters.get("alpha", 0.05)].index
            )
    ...
    def compare_iterations(
        ...
        try:
            if modality_name not in self.iterations:
                raise ValueError(
                    f"No iterations tracked for modality '{modality_name}'"
                )
            ...
            overlap_percent = (
                len(overlap) / max(len(sig1), len(sig2)) * 100
                if max(len(sig1), len(sig2)) > 0
                else 0
            )
    ...
    def get_iteration_summary(self, modality_name: str) -> str:
        ...
        try:
            ...
            sorted_iterations = sorted(
                modality_iterations.items(), key=lambda x: x[1]["id"]
            )
            ...
            for iter_id, iter_data in sorted_iterations:
                ...
                summary += (
                    f"• Significant genes: {iter_data['n_significant_genes']:,}\n"
                )
    ...
    def suggest_next_iteration(
        self, modality_name: str, current_results: Optional[pd.DataFrame] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            if "+" not in formula:
                ...
                suggestions.append(
                    "Try interaction terms if condition effects vary by batch"
                )
    ...
    def export_iteration_history(
        self, modality_name: str, output_path: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            export_data = {
                ...
                "comparisons": {
                    k: v
                    for k, v in self.comparison_cache.items()
            ...
            self.logger.info(
                f"Exported iteration history for '{modality_name}' to {output_path}"
            )
            return output_path
    ...
    def import_iteration_history(self, import_path: str) -> str:
        """
        ...
        try:
            ...
            if import_data["iterations"]:
                max_id = max(
                    int(iter_data["id"])
                    for iter_data in import_data["iterations"].values()
                )
    ...
    def get_best_iteration(
        self, modality_name: str, metric: str = "n_significant_genes"
        ...
        try:
            if (
                modality_name not in self.iterations
                or not self.iterations[modality_name]
            ):
                return None
    ...
    def cleanup_iterations(self, modality_name: str, keep_last_n: int = 5) -> int:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Cleaned up {removed_count} old iterations for '{modality_name}'"
            )
    ...
    def generate_comparison_report(
        ...
        try:
            ...
            report += (
                f"• **Total unique genes**: {overlap_stats['total_unique_genes']:,}\n\n"
            )
    ...
    def get_iteration_by_name(
        self, modality_name: str, iteration_name: str
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def list_iterations(self, modality_name: str) -> List[str]:
        ...
        return [
            iter_data["name"] for iter_data in self.iterations[modality_name].values()
        ]
    ...
    def clear_iterations(self, modality_name: str) -> int:
        ...
        cache_keys_to_remove = [
            k for k in self.comparison_cache.keys() if k.startswith(modality_name)
        ]
        for cache_key in cache_keys_to_remove:
            del self.comparison_cache[cache_key]
...
class IterationComparisonResult:
    ...
    def format_summary(self) -> str:
        ...
        try:
            ...
            summary += (
                f"• Unique genes: {stats['unique_to_1']:,} + {stats['unique_to_2']:,}\n"
            )
