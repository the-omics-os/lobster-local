...
class SingleCellWithRawDataRule(PipelineRule):
    ...
    def evaluate(self, context: PipelineContext) -> Optional[PipelineType]:
        ...
        if is_single_cell and raw_available and context.data_availability == DataAvailability.MINIMAL:
...
class SupplementaryFilesRule(PipelineRule):
    ...
    def evaluate(self, context: PipelineContext) -> Optional[PipelineType]:
        ...
        no_matrices = not context.has_file("processed_matrix") and not context.has_file("raw_UMI_like_matrix")
...
class PipelineStrategyEngine:
    ...
    def register_rule(self, rule: PipelineRule):
        ...
        logger.debug(f"Registered rule: {rule.__class__.__name__} (priority: {rule.get_priority()})")
    ...
    def determine_pipeline(self, context: PipelineContext) -> Tuple[PipelineType, str]:
        ...
        for rule in self.rules:
            ...
            if pipeline_type is not None:
                ...
                logger.info(f"Selected pipeline: {pipeline_type.name} via {rule.__class__.__name__}")
    ...
    def get_pipeline_functions(
        ...
        return pipeline_map.get(
            pipeline_type,
            pipeline_map[PipelineType.FALLBACK]
        )
