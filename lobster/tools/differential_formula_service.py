...
class DifferentialFormulaService:
    ...
    def parse_formula(
        ...
        try:
            ...
            self.logger.info(f"Formula parsed successfully: {len(terms)} terms, rank ≈ {formula_components['design_rank']}")
        ...
        try:
            ...
            for term in formula_components['predictor_terms']:
                if term['type'] == 'main_effect':
                    design_df = self._add_main_effect(design_df, term, metadata, formula_components)
                elif term['type'] == 'interaction':
                    design_df = self._add_interaction(design_df, term, metadata, formula_components)
    ...
    def _validate_variables(self, terms: List[Dict[str, Any]], metadata: pd.DataFrame) -> None:
            ...
            if pd.api.types.is_numeric_dtype(series):
                ...
                if reference_levels and var in reference_levels:
                    ...
                    if ref_level in levels:
                        levels = [ref_level] + [l for l in sorted(levels) if l != ref_level]
                    else:
                        self.logger.warning(f"Reference level '{ref_level}' not found for variable '{var}'")
    ...
    def _add_interaction(
        ...
        if len(variables) == 2:
            ...
            raise DesignMatrixError(f"Higher-order interactions (>{len(variables)}) not yet supported")
    ...
    def _validate_design_matrix(self, design_matrix: np.ndarray, column_names: List[str]) -> None:
    ...
    def validate_experimental_design(
        ...
        try:
            ...
            n_samples = len(metadata)
            if n_samples < 6:
                validation_results['warnings'].append(f"Small sample size: {n_samples} samples")
            ...
            if missing_vars:
                validation_results['warnings'].append(f"Missing values in: {missing_vars}")
