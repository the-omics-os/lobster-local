...
class AnnotationTemplateService:
    ...
    def _create_pbmc_template(self) -> Dict[str, CellTypeTemplate]:
        ...
        return {
            ...
            "NK cells": CellTypeTemplate(
                ...
                alternative_names=["Natural killer cells", "Large granular lymphocytes"]
            ),
            ...
            "Monocytes CD16+": CellTypeTemplate(
                ...
                alternative_names=["Non-classical monocytes", "CD16+ monocytes", "FCGR3A+ monocytes"]
            ),
    ...
    def _create_kidney_template(self) -> Dict[str, CellTypeTemplate]:
        ...
        return {
            "Podocytes": CellTypeTemplate(
                ...
                alternative_names=["Glomerular epithelium", "Visceral epithelial cells"]
            ),
    ...
    def _create_tumor_template(self) -> Dict[str, CellTypeTemplate]:
        ...
        return {
            "Tumor cells": CellTypeTemplate(
                ...
                alternative_names=["Cancer cells", "Malignant cells", "Neoplastic cells"]
            ),
    ...
    def get_template(self, tissue_type: TissueType) -> Optional[Dict[str, CellTypeTemplate]]:
        """
    ...
    def get_markers_for_cell_type(self, 
    ...
    def search_cell_types_by_marker(self, 
                                   marker_gene: str, 
                                   tissue_type: Optional[TissueType] = None) -> List[Tuple[TissueType, str]]:
        """
        ...
        for tissue in tissues_to_search:
            ...
            for cell_type_name, cell_type_template in template.items():
                if marker_gene.upper() in [marker.upper() for marker in cell_type_template.markers]:
    ...
    def create_custom_template(self, 
                             name: str, 
                             cell_types: Dict[str, CellTypeTemplate]) -> None:
        """
    ...
    def apply_template_to_clusters(self, 
                                 adata,
                                 tissue_type: TissueType,
                                 cluster_col: str = 'leiden',
                                 expression_threshold: float = 0.5) -> Dict[str, str]:
        """
    ...
    def _calculate_cell_type_score(self, 
                                 cluster_data, 
                                 cell_type_template: CellTypeTemplate,
                                 expression_threshold: float) -> float:
        """
        ...
        """
        available_markers = [marker for marker in cell_type_template.markers 
        ...
        for marker in available_markers:
            ...
            pct_expressing = np.sum(marker_expr > expression_threshold) / len(marker_expr)
        ...
        if format.lower() == 'json':
            ...
            for cell_type_name, cell_type_template in template.items():
                rows.append({
                    ...
                    'confidence_threshold': cell_type_template.confidence_threshold,
                    'description': cell_type_template.description,
                    'alternative_names': ', '.join(cell_type_template.alternative_names)
                })
        ...
        if format.lower() == 'json':
            ...
            for cell_type_name, cell_type_data in import_data['cell_types'].items():
                cell_types[cell_type_name] = CellTypeTemplate(
                    ...
                    confidence_threshold=cell_type_data.get('confidence_threshold', 0.5),
