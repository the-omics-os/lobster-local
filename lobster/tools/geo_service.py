...
class GEOService:
    ...
    def __init__(self, data_manager: DataManager, cache_dir: Optional[str] = None, console=None):
        ...
        """
        if GEOparse is None:
            raise ImportError("GEOparse is required but not installed. Please install with: pip install GEOparse")
    ...
    def download_dataset(self, query: str) -> str:
        ...
        try:
            ...
            if existing_data is not None:
                ...
                self.data_manager.set_data(data=combined_matrix, metadata=saved_metadata)
...
⚡ Ready for immediate analysis!"""
            ...
            enhanced_metadata = {
                ...
                'data_source': 'supplementary_files' if supplementary_data is not None else 'individual_samples',
            ...
            saved_file = self._save_processed_matrix_to_workspace(gse_id, combined_matrix, enhanced_metadata)
            ...
            return self._format_download_response(gse_id, combined_matrix, metadata, sample_count, saved_file)
    ...
    def _process_supplementary_files(self, gse, gse_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            expression_files = [f for f in suppl_files if any(ext in f.lower() for ext in ['.txt.gz', '.csv.gz', '.tsv.gz', '.h5', '.h5ad'])]
            ...
            logger.warning(f"No suitable expression files found in supplementary files for {gse_id}")
            return None
        ...
        try:
            ...
            if nested_archives:
                ...
                for archive_path in nested_archives:
                    try:
                        sample_id = archive_path.stem.split('.')[0]  # Extract sample ID from filename
                        ...
                        if matrix is not None:
                            ...
                            logger.info(f"Successfully parsed 10X data for {sample_id}: {matrix.shape}")
                        
                    except Exception as e:
                        logger.warning(f"Failed to extract/parse {archive_path.name}: {e}")
                        continue
            ...
            for file_path in extract_dir.rglob('*'):
                if file_path.is_file() and any(ext in file_path.name.lower() for ext in ['.txt', '.csv', '.tsv', '.gz']):
                ...
                for file_path in expression_files[:3]:  # Try top 3 largest files
                    try:
                        ...
                        if matrix is not None and matrix.shape[0] > 0 and matrix.shape[1] > 0:
                            logger.info(f"Successfully parsed expression matrix: {matrix.shape}")
                            return matrix
                    except Exception as e:
    ...
    def _download_and_parse_file(self, file_url: str, gse_id: str) -> Optional[pd.DataFrame]:
        """
    ...
    def _parse_10x_data(self, extract_dir: Path, sample_id: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            logger.info(f"Found 10X files - Matrix: {matrix_file.name}, Barcodes: {barcodes_file.name if barcodes_file else 'None'}, Features: {features_file.name if features_file else 'None'}")
    ...
    def _parse_expression_file(self, file_path: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if file_path.name.endswith('.gz'):
                opener = lambda: pd.read_csv(file_path, compression='gzip', sep='\t', index_col=0, low_memory=False)
            else:
                opener = lambda: pd.read_csv(file_path, sep='\t', index_col=0, low_memory=False)
            ...
            try:
                if file_path.name.endswith('.gz'):
                    df = pd.read_csv(file_path, compression='gzip', sep=',', index_col=0, low_memory=False)
                else:
    ...
    def _parse_matrix_market_file(self, matrix_file: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            sample_id = matrix_file.name.replace('_matrix.mtx.gz', '').replace('_matrix.mtx', '').replace('matrix.mtx.gz', '').replace('matrix.mtx', '')
            if not sample_id:
            ...
            barcode_patterns = ['barcodes.tsv.gz', 'barcodes.tsv', f'{sample_id}_barcodes.tsv.gz', f'{sample_id}_barcodes.tsv']
            feature_patterns = ['features.tsv.gz', 'features.tsv', 'genes.tsv.gz', 'genes.tsv', 
            ...
            logger.info(f"Found files - Barcodes: {barcodes_file.name if barcodes_file else 'None'}, Features: {features_file.name if features_file else 'None'}")
        ...
        try:
            ...
            if hasattr(gse, 'gpls'):
                ...
                for gpl_id, gpl in gse.gpls.items():
                    platforms[gpl_id] = {
                        'title': getattr(gpl, 'metadata', {}).get('title', [''])[0] if hasattr(gpl, 'metadata') else '',
                        'organism': getattr(gpl, 'metadata', {}).get('organism', [''])[0] if hasattr(gpl, 'metadata') else '',
                        'technology': getattr(gpl, 'metadata', {}).get('technology', [''])[0] if hasattr(gpl, 'metadata') else ''
                    }
    ...
    def _get_sample_info(self, gse) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            if hasattr(gse, 'gsms'):
                for gsm_id, gsm in gse.gsms.items():
                    sample_info[gsm_id] = {
                        'title': getattr(gsm, 'metadata', {}).get('title', [''])[0] if hasattr(gsm, 'metadata') else '',
                        'platform': getattr(gsm, 'metadata', {}).get('platform_id', [''])[0] if hasattr(gsm, 'metadata') else '',
    ...
    def _download_sample_matrices(self, sample_info: Dict[str, Dict[str, Any]], gse_id: str) -> Dict[str, Optional[pd.DataFrame]]:
        """
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_sample = {
                executor.submit(self._download_single_sample, gsm_id, info, gse_id): gsm_id 
            ...
            for future in as_completed(future_to_sample):
                ...
                try:
                    ...
                    if matrix is not None:
                        logger.info(f"Successfully downloaded matrix for {gsm_id}: {matrix.shape}")
                    else:
    ...
    def _download_single_sample(self, gsm_id: str, sample_info: Dict[str, Any], gse_id: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if hasattr(gsm, 'metadata') and 'supplementary_file' in gsm.metadata:
                ...
                for suppl_url in suppl_files:
                    if any(ext in suppl_url.lower() for ext in ['.txt', '.csv', '.tsv', '.gz']):
                        try:
                            matrix = self._download_supplementary_file(suppl_url, gsm_id)
                            if matrix is not None:
                                return matrix
                        except Exception as e:
                            logger.warning(f"Failed to download supplementary file {suppl_url}: {e}")
                            continue
    ...
    def _download_supplementary_file(self, url: str, gsm_id: str) -> Optional[pd.DataFrame]:
        """
            ...
            for sep in ['\t', ',']:
                try:
                    if local_file.suffix == '.gz':
                        df = pd.read_csv(local_file, sep=sep, compression='gzip', index_col=0)
                    else:
                        df = pd.read_csv(local_file, sep=sep, index_col=0)
                    
                    if df.shape[0] > 0 and df.shape[1] > 0:
                        logger.info(f"Successfully parsed supplementary file for {gsm_id}: {df.shape}")
                        return df
                except:
                    continue
    ...
    def _validate_matrices(self, sample_matrices: Dict[str, Optional[pd.DataFrame]]) -> Dict[str, pd.DataFrame]:
        """
    ...
    def _concatenate_matrices(self, validated_matrices: Dict[str, pd.DataFrame], gse_id: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            logger.info(f"Concatenating {len(validated_matrices)} matrices for {gse_id}")
            ...
            for gsm_id, matrix in validated_matrices.items():
                ...
                aligned_matrix.index = [f"{gsm_id}_{idx}" for idx in aligned_matrix.index]
    ...
    def _check_workspace_for_processed_data(self, gse_id: str) -> Optional[Tuple[pd.DataFrame, Dict[str, Any]]]:
        """
    ...
    def _save_processed_matrix_to_workspace(self, gse_id: str, combined_matrix: pd.DataFrame, 
        ...
        try:
            ...
            # Save metadata
            metadata_path = self.data_manager.data_dir / f"{gse_id}_processed_{timestamp}_metadata.json"
    ...
    def _format_download_response(self, gse_id: str, combined_matrix: pd.DataFrame, 
                                 metadata: Dict[str, Any], sample_count: int, saved_file: str = None) -> str:
        """
        ...
        """
        study_title = metadata.get('title', ['N/A'])[0] if isinstance(metadata.get('title'), list) else metadata.get('title', 'N/A')
        organism = metadata.get('organism', ['N/A'])[0] if isinstance(metadata.get('organism'), list) else metadata.get('organism', 'N/A')
