...
class CloudLobsterClient(BaseClient):
    ...
    def query(self, user_input: str, stream: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.endpoint}/query", json=payload, timeout=self.timeout
            )
    ...
    def get_usage(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.session.get(f"{self.endpoint}/usage", timeout=10)
            response.raise_for_status()

            return response.json()

        except Exception as e:
            return {"error": f"Could not get usage info: {e}", "success": False}
    ...
    def list_models(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.session.get(f"{self.endpoint}/models", timeout=10)
            response.raise_for_status()

            return response.json()

        except Exception as e:
            return {"error": f"Could not list models: {e}", "success": False}
    ...
    def read_file(self, filename: str) -> Optional[str]:
        ...
        try:
            response = self.session.get(
                f"{self.endpoint}/workspace/files/{filename}", timeout=30
            )
            response.raise_for_status()
