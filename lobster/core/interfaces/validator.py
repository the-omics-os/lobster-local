...
class IValidator(ABC):
    ...
    def validate_layers(
        self, adata: anndata.AnnData, expected_layers: Optional[List[str]] = None
    ) -> ValidationResult:
        """
    ...
    def validate_data_quality(self, adata: anndata.AnnData) -> ValidationResult:
        ...
        if hasattr(adata.X, "isnan"):
            ...
            if nan_count > 0:
                ...
                if nan_percentage > 50:
                    result.add_warning(
                        f"High proportion of NaN values in X matrix: {nan_percentage:.1f}%"
                    )
                else:
        ...
        if hasattr(adata.X, "min"):
            ...
            if min_val < 0:
                result.add_warning(
                    f"Negative values found in X matrix (minimum: {min_val})"
                )
