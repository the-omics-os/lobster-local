...
class PseudobulkAdapter(BaseAdapter):
    ...
    def __init__(self, strict_validation: bool = False):
        ...
        # Create validator
        self.validator = PseudobulkSchema.create_validator(
            ...
            if validate_schema:
                ...
                if validation_result.has_errors():
                    if self.strict_validation:
                        ...
                        self.logger.warning(f"Pseudobulk validation warnings: {validation_result.get_warning_summary()}")
            
            self.logger.info(f"Loaded pseudobulk data: {adata.n_obs} pseudobulk samples × {adata.n_vars} genes")
            return adata
        ...
        **kwargs
        ...
        try:
            if format_type == 'h5ad':
                ...
                raise AdapterError(f"Unsupported file format for pseudobulk data: {format_type}")
        except Exception as e:
    ...
    def _parse_pseudobulk_sample_ids(self, sample_ids: pd.Index) -> pd.DataFrame:
        ...
        for sample_id in sample_ids:
            ...
            parts = str(sample_id).rsplit('_', 1)  # Split from right to handle underscores in sample names
    ...
    def _add_pseudobulk_metadata(
        ...
        if aggregation_metadata:
            if 'pseudobulk_params' in aggregation_metadata:
                adata.uns['pseudobulk_params'] = aggregation_metadata['pseudobulk_params']
            
            if 'aggregation_stats' in aggregation_metadata:
                adata.uns['aggregation_stats'] = aggregation_metadata['aggregation_stats']
    ...
    def _calculate_pseudobulk_metrics(self, adata: anndata.AnnData) -> anndata.AnnData:
        ...
        if 'n_pseudobulk_samples' not in adata.var.columns:
            adata.var['n_pseudobulk_samples'] = (adata.X > 0).sum(axis=0)
        
        if 'mean_aggregated_counts' not in adata.var.columns:
            adata.var['mean_aggregated_counts'] = np.array(adata.X.mean(axis=0)).flatten()
        
        if 'total_aggregated_counts' not in adata.var.columns:
            adata.var['total_aggregated_counts'] = np.array(adata.X.sum(axis=0)).flatten()
        ...
        if 'total_aggregated_counts' not in adata.obs.columns:
            adata.obs['total_aggregated_counts'] = np.array(adata.X.sum(axis=1)).flatten()
    ...
    def _validate_pseudobulk_specific(self, adata: anndata.AnnData) -> ValidationResult:
        ...
        if 'n_cells_aggregated' in adata.obs.columns:
            ...
            low_cell_samples = (cell_counts < self.qc_thresholds['min_cells_per_pseudobulk']).sum()
        ...
        if all(col in adata.obs.columns for col in ['sample_id', 'cell_type', 'condition']):
            self._check_experimental_balance(adata, result)
        
        return result

    def _check_experimental_balance(self, adata: anndata.AnnData, result: ValidationResult):
        """Check experimental design balance."""
        design_counts = adata.obs.groupby(['condition', 'cell_type']).size().unstack(fill_value=0)
        
        # Check if all condition-celltype combinations have samples
        zero_combinations = (design_counts == 0).sum().sum()
        if zero_combinations > 0:
            result.add_warning(f"{zero_combinations} condition-celltype combinations have no samples")
    ...
    def get_quality_metrics(self, adata: anndata.AnnData) -> Dict[str, Any]:
        ...
        if 'n_pseudobulk_samples' in adata.var.columns:
            ...
            metrics.update({
                'genes_expressed_all_samples': int((n_samples_expressing == adata.n_obs).sum()),
                'genes_expressed_half_samples': int((n_samples_expressing >= adata.n_obs / 2).sum()),
    ...
    def create_aggregated_data(
        ...
        try:
            ...
            if celltype_col not in single_cell_adata.obs.columns:
                raise AdapterError(f"Cell type column '{celltype_col}' not found in obs")
