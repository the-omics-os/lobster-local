...
class PseudobulkSchema:
        ...
        return {
            ...
            "uns": {
                ...
                "optional": [
                    ...
                    "contact_address", "contact_city", "contact_country", "contact_department",
                    "contact_email", "contact_institute", "contact_name", "contact_phone",
                    "contact_zip/postal_code", "contributor", "geo_accession", "last_update_date",
                    "overall_design", "platform_id", "platform_taxid", "platforms", "pubmed_id",
                    "relation", "sample_id", "sample_taxid", "samples", "status", "submission_date",
                    "summary", "supplementary_file", "title", "type", "web_link"
                ],
                "types": {
    ...
    def create_validator(
        ...
        validator = FlexibleValidator(
            schema=schema,
            name="PseudobulkValidator",
            ignore_warnings=ignore_set
        )

        # Add pseudobulk-specific validation rules
        validator.add_custom_rule("check_pseudobulk_structure", _validate_pseudobulk_structure)
        validator.add_custom_rule("check_aggregation_consistency", _validate_aggregation_consistency)
        validator.add_custom_rule("check_cell_counts", _validate_cell_counts)
        validator.add_custom_rule("check_aggregation_params", _validate_aggregation_params)
...
def _validate_pseudobulk_structure(adata) -> ValidationResult:
    ...
    if 'sample_id' in adata.obs.columns and 'cell_type' in adata.obs.columns:
        composite_ids = adata.obs['sample_id'].astype(str) + '_' + adata.obs['cell_type'].astype(str)
        duplicates = composite_ids.duplicated().sum()
        if duplicates > 0:
            result.add_error(f"Found {duplicates} duplicate sample_id + cell_type combinations")
...
def _validate_aggregation_consistency(adata) -> ValidationResult:
    ...
    if 'n_cells_aggregated' in adata.obs.columns:
        ...
        if invalid_counts > 0:
            result.add_error(f"Found {invalid_counts} pseudobulk samples with zero or negative cell counts")
        
        # Check for very low cell counts
        low_counts = (cell_counts < 10).sum()
        if low_counts > 0:
            result.add_warning(f"Found {low_counts} pseudobulk samples with <10 cells (may be unreliable)")
        ...
        if len(cell_counts) > 1:
            ...
            if q99 > 10 * q1:
                result.add_warning(f"Large variation in cell counts (99th percentile: {q99:.0f}, 1st percentile: {q1:.0f})")
...
def _validate_cell_counts(adata) -> ValidationResult:
    ...
    if not all(col in adata.obs.columns for col in ['sample_id', 'cell_type', 'n_cells_aggregated']):
        return result
    
    # Check balance across conditions if available
    if 'condition' in adata.obs.columns:
        condition_counts = adata.obs.groupby(['condition', 'cell_type'])['n_cells_aggregated'].sum()
        ...
        for condition in conditions:
            for cell_type in cell_types:
                if (condition, cell_type) not in condition_counts.index:
                    result.add_warning(f"Missing data for condition '{condition}' and cell type '{cell_type}'")
    ...
    if duplicates > 0:
        result.add_error(f"Found {duplicates} sample-celltype combinations with multiple entries")
...
def _validate_aggregation_params(adata) -> ValidationResult:
    ...
    if 'aggregation_method' in params:
        ...
        if params['aggregation_method'] not in valid_methods:
            result.add_warning(f"Unexpected aggregation method: {params['aggregation_method']} (expected one of {valid_methods})")
