...
class TerminalCallbackHandler(BaseCallbackHandler):
    ...
    def _display_agent_event(self, event: AgentEvent):
        ...
        if event.type == EventType.AGENT_START:
            ...
            self.console.print(f"[green]   ✓ {agent_display} complete{duration_str}[/green]")
            ...
            if event.content:
                self.console.print(f"[dim]   Task: {self._truncate_content(event.content)}[/dim]")
    ...
    def _display_tool_event(self, event: AgentEvent):
        ...
        if event.type == EventType.TOOL_START:
            ...
            if self.verbose and event.content:
                self.console.print(f"[dim]      Input: {self._truncate_content(event.content)}[/dim]")
        
        elif event.type == EventType.TOOL_COMPLETE:
            duration_str = ""
            if event.duration:
                duration_str = f" [{self._format_duration(event.duration)}]"
            self.console.print(f"[green]      ✓ {tool_name} complete{duration_str}[/green]")
            if self.verbose and event.content:
                self.console.print(f"[dim]      Result: {self._truncate_content(event.content)}[/dim]")
        
        elif event.type == EventType.TOOL_ERROR:
            self.console.print(f"[red]      ✗ {tool_name} failed: {event.content}[/red]")
    ...
    def on_llm_end(self, response: LLMResult, **kwargs) -> None:
        ...
        if self.current_agent in self.start_times:
            duration = (datetime.now() - self.start_times[self.current_agent]).total_seconds()
    ...
    def display_summary(self):
        ...
        for agent, stats in agent_stats.items():
            table.add_row(
                ...
                self._format_duration(stats["duration"]) if stats["duration"] > 0 else "N/A"
            )
...
class SimpleTerminalCallback(TerminalCallbackHandler):
    ...
    def on_tool_use(self, tool_name: str, tool_input: Any):
        ...
        self.console.print(f"[yellow]   🔧 Using {tool_name}[/yellow]")
        if self.verbose:
            self.console.print(f"[dim]      {self._truncate_content(str(tool_input))}[/dim]")
