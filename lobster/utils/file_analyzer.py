...
class FileAnalyzer:
    ...
    def analyze_file_metadata(cls, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            is_data_file = (
                actual_extension in cls.DATA_FILE_EXTENSIONS or file_extension == ".gz"
            )
    ...
    def _analyze_h5ad_data(cls, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            adata = sc.read_h5ad(
                file_path, backed="r"
            )  # Read in backed mode for metadata only
            ...
            with h5py.File(file_path, "r") as f:
                ...
                def collect_datasets(name, obj):
                    if isinstance(obj, h5py.Dataset):
                        datasets.append(
                            {"name": name, "shape": obj.shape, "dtype": str(obj.dtype)}
                        )
                ...
                if datasets:
                    ...
                    if not main_dataset:
                        ...
                        main_dataset = max(
                            datasets, key=lambda x: x["shape"][0] if x["shape"] else 0
                        )

                if main_dataset and main_dataset["shape"]:
                    row_count = main_dataset["shape"][0]
                    column_count = (
                        main_dataset["shape"][1]
                        if len(main_dataset["shape"]) > 1
                        else 1
                    )
                else:
                    row_count = None
            ...
            if wb.worksheets:
                ...
                if row_count and column_count:
                    ...
                    if row_count > 1:
                        ...
                        second_row_values = (
                            [cell.value for cell in ws[2]] if row_count > 1 else []
                        )
                        has_header = cls._detect_excel_header(
                            first_row_values, second_row_values
                        )
    ...
    @classmethod
    def _analyze_compressed_text_data(
        cls, file_path: Path, file_format: str
    ) -> Dict[str, Any]:
    ...
    @classmethod
    def _analyze_compressed_binary_data(
        cls, file_path: Path, file_format: str
    ) -> Dict[str, Any]:
    ...
    def _detect_header(cls, first_line: str, second_line: str, delimiter: str) -> bool:
        ...
        try:
            ...
            first_numeric = sum(
                1 for val in first_values if cls._is_numeric(val.strip())
            )
            second_numeric = sum(
                1 for val in second_values if cls._is_numeric(val.strip())
            )
    ...
    @classmethod
    def _detect_excel_header(
        cls, first_row_values: list, second_row_values: list
    ) -> bool:
        ...
        try:
            first_numeric = sum(
                ...
                if val is not None and cls._is_numeric(str(val))
            )
            second_numeric = sum(
                1
                for val in second_row_values
                if val is not None and cls._is_numeric(str(val))
            )
            ...
            if len(first_row_values) > 0:
                ...
                second_numeric_ratio = (
                    second_numeric / len(second_row_values) if second_row_values else 0
                )
...
def get_workspace_files_metadata(
    workspace_path: Union[str, Path], directory: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
                ...
                if file_path.is_file():
                    try:
                        ...
                        metadata["directory"] = str(
                            file_path.parent.relative_to(workspace_path)
                        )
