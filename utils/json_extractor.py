...
def extract_json_from_text(text: str, fallback_response: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for pattern in code_block_patterns:
        ...
        for match in matches:
            try:
                parsed = json.loads(match.strip())
                logger.debug(f"Successfully extracted JSON from code block")
                return parsed
            except json.JSONDecodeError:
                continue
    ...
    for line in lines:
        ...
        if line.startswith('{') and line.endswith('}'):
            try:
                ...
                logger.debug(f"Successfully extracted JSON from line")
                return parsed
            except json.JSONDecodeError:
                continue
                ...
                if end_idx > start_idx:
                    try:
                        ...
                        if candidate.startswith('{') and candidate.endswith('}'):
                            ...
                            logger.debug(f"Successfully extracted JSON from substring")
                            return parsed
                    except json.JSONDecodeError:
                        continue
